...
WebServer::WebServer() : is_close_(false), timer_(new HeapTimer), epoller_(new Epoller()) {
    ...
    int conn_pool_max_num = json["mysql"]["max_conn"].toNumber();
    int conn_pool_min_num = json["mysql"]["min_conn"].toNumber();
    int sql_time_out = json["mysql"]["time_out"].toNumber();
    unsigned int conn_pool_max_Idle_time = json["mysql"]["max_Idle_time"].toNumber();
    ConnectPool::get_connect_pool()->init(sql_IP, sql_user, sql_pwd, db_name, port, conn_pool_max_num, conn_pool_min_num, sql_time_out, conn_pool_max_Idle_time);

    int minThreadNum = json["thread"]["minThreadNum"].toNumber();
    int maxThreadNum = json["thread"]["maxThreadNum"].toNumber();
    std::unique_ptr<ThreadPool> new_thread_pool(new ThreadPool(minThreadNum, maxThreadNum));
    ...
    if (open_log) {
        ...
        if (is_close_) {
            ...
            LOG_INFO("SqlConnPool init num: %d, ThreadPool init num: %d", conn_pool_min_num, minThreadNum);
        }
    }
}
...
WebServer::WebServer(
        ...
        const char* db_name, int conn_pool_max_num, int conn_pool_min_num, int sql_time_out, unsigned int conn_pool_max_Idle_time, int thread_num,
    ...
    if (open_log) {
        ...
        if (is_close_) {
            ...
            LOG_INFO("LogSys level: %d", log_level);
            LOG_INFO("srcDir: %s", HttpConn::src_dir_);
            LOG_INFO("SqlConnPool init num: %d, ThreadPool num: %d", conn_pool_max_num, thread_num);
        }
    }
}
